	...
	SubShader{
		Pass{
			...
			float4 frag(vertexOutput IN) : COLOR
			{
				...
				float4 texE = tex2D(_EmitMap, IN.tex.xy * _EmitMap_ST.xy + _EmitMap_ST.zw);
				...
				float3 lightFinal = UNITY_LIGHTMODEL_AMBIENT + diffuseReflect + (specularReflect * tex.a) + rimLighting + (texE.xyz * _EmitStrength);
		...
		Pass{
			...
			float4 frag(vertexOutput IN) : COLOR
			{
				...
				float3 lightFinal = diffuseReflect + (specularReflect * tex.a) + rimLighting;
